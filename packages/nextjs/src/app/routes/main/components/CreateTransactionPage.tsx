...
import { BaseContract, BigNumber, Contract, Signer, ethers } from "ethers";
...
export const CreateTransactionPage: FC<CreateTransactionPageProps> = props => {
  ...
  const inputPlaceholder = "" + (nonce ? nonce.toNumber() : "loading...");
  ...
  const createTransaction = async () => {
    ...
    const signature = await signer?.signMessage(
      ethers.utils.isHexString(newHash) ? ethers.utils.arrayify(newHash) : newHash,
    );
        ...
        <div style={{ margin: 8 }}>
          ...
          <div style={{ margin: 8, padding: 8 }}>
            <Select value={methodName} disabled={selectDisabled} style={{ width: "100%" }} onChange={setMethodName}>
              ...
              <Option disabled={true} value="addSigner">
                addSigner()
              </Option>
              <Option disabled={true} value="removeSigner">
                removeSigner()
              </Option>
            </Select>
          </div>
          ...
          <Button style={{ marginTop: 32 }} disabled={!isCreateTxnEnabled} onClick={createTransaction}>
            Create
          </Button>
        </div>
